...
def solution(eps, wd, cavity_levels, transmon_levels):
    ...
    output = mcsolve(H, rho0, taus, c_op_list, [a.dag() * a] + projections, ntraj=50)
...
if __name__ == '__main__':
    ...
    plt.subplot(2, 1, 1)
    plt.plot(output.times / (2 * 3.146 * 1000), output.expect[0]);
    plt.xlabel('Time');
    plt.ylabel('Photons');
    plt.subplot(2, 1, 2)
    probabilities = np.array([output.expect[x] for x in range(1, cavity_levels + 1)]);
